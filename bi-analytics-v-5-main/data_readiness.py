...
def build_data_readiness_report(
    ...
    # БДДС / лот
    for n in ("БДДС (расходы)", "БДДС", "Бюджет по лотам"):
    ...
    for n in ("БДР (расходы)", "БДР", "Бюджет план/факт", "Утвержденный бюджет", "БДДС расходы (план, факт, уточненный план)", "Прогнозный бюджет"):
        h = _has_budget_msp_or_1c(pd_, ref1c if _df_ok(ref1c) else None)
        if h == "ok":
            add(n, "ok", "MSP/1С", "Источник сумм доступен; при расхождении — согласовать джойн с заказчиком.")
        else:
            add(n, "warn" if n in ("БДДС расходы (план, факт, уточненный план)", "Прогнозный бюджет") else "err", "MSP/1С", "Возможен пустой отчёт: нет бюджетных колонок / нет 1С.")
