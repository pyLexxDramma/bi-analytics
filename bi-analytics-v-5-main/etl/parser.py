...
Соглашение об именах:
  ...
  other_{project}_{DD-MM-YYYY}_rd.csv     → тип 'rd_plan',    проект из имени
...
def _extract_project_from_rd(name_lower: str) -> str:
    ...
    """
    tail = re.sub(r"^other_", "", name_lower)
    tail = _DATE_DDMMYYYY.sub("", tail)
    tail = _DATE_YYYYMMDD.sub("", tail)
    tail = re.sub(r"_rd(\.csv)?$", "", tail)
    tail = re.sub(r"\.csv$", "", tail)
    tail = tail.strip("_- ")
    return tail or "unknown"
...
def detect_file(path: Path) -> dict:
    ...
    if name_lower.endswith(".json") and (
        name_lower.startswith("1с") or name_lower.startswith("1c")
    ):
