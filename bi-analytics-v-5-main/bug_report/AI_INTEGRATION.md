...
3. По `category` / `priority` кладём карточку в нужный список Trello + метку.
...
Категории → списки Trello:

| category | Смысл | Список |
|----------|--------|--------|
| `urgent` | блокер / недоступность | Срочно |
...
| `other` | непонятно | На разбор (triage) |
