...
def main() -> None:
    ...
    if _kontr is not None and not contractors.empty and "contractor" in contractors.columns:
        mask = contractors.apply(
            lambda r: contractor_in_kontr("", str(r.get("contractor", "")), _kontr),
            axis=1,
        )
        contractors = contractors.loc[mask].copy()
