...
SESSIONS_DB_PATH = Path(os.getenv("SESSIONS_DB_PATH", str(_WEB_DIR / "sessions.db")))
RULES_MD_PATH = Path(os.getenv("AI_ASSISTANT_RULES_PATH", str(_WEB_DIR / "AI_ASSISTANT_RULES.md")))
APP_LOGO_PATH = _WEB_DIR / "logo.svg"
...
def _read_sse_events_once() -> list[dict[str, Any]]:
    ...
    try:
        opencode_url = str(st.session_state.get("runtime_opencode_url") or get_opencode_base_url())
    except Exception:
        return events
...
def create_session(auto: bool = False) -> bool:
    ok, status, data, raw_text = fetch_json(
        path=f"/session?directory={quote(DEFAULT_XCA_WORKSPACE, safe='')}",
        method="POST",
...
def app(on_back_requested: Any | None = None, *, embedded: bool = False) -> None:
    main(on_back_requested=on_back_requested, embedded=embedded)


def main(on_back_requested: Any | None = None, *, embedded: bool = False) -> None:
    if not embedded:
    ...
    if not str(st.session_state.get("runtime_opencode_url", "")).strip():
        st.session_state.runtime_opencode_url = get_opencode_base_url()
    ensure_auto_session()
    render_sidebar()
